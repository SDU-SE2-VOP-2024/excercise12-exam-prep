...
the `divisor` argument (Hint: `x % divisor == 0`).
...
> Hint: Use a `Set` when generating the numbers. When the set has the desired size, you can fill in an array with values
> form the set.

> Hint: throw an `IllegalArgumentException` if the size is greater than max.
